...
int main(int argc, char** argv) {
    ...
    int steps = 180; float jointStiffness = 600.0f;
    ...
    auto maxSpeed = [&]()->float{
        float mx=0; for (auto& kv:byObj) for (auto& n:kv.second)
    ...
    for (auto& b : rig.bones) {
        ...
        if (nm=="pelvis.L"||nm=="pelvis.R"||nm=="lumbar") {
            for (auto& n : byObj[b.objectId]) { n->physics.velocity.setZero(); n->setStatic(true); }
        }
    }
    ...
    double bondStretchSum=0; float bondStretchMax=0; long jointBondCount=0;
    for (auto& kv : byObj) for (auto& n : kv.second) {
        for (auto& bd : n->physics.bonds) {
            ...
            bondStretchSum += rel; bondStretchMax = std::max(bondStretchMax, rel);
            ++jointBondCount;
        }
    }
    float avgBondStretch = jointBondCount ? (float)(bondStretchSum/jointBondCount) : 0.0f;

    float maxTravel=0, sumTravel=0; int nJ=0;
    std::cout << "\nJoint            childMove\n";
    for (auto& j : rig.joints) {
        if (j.parentObjId < 0) continue;
        float travel=(centroid(j.childObjId)-childStart[j.name]).norm();
        maxTravel=std::max(maxTravel,travel); sumTravel+=travel; ++nJ;
