...
void Octree<T, GasT, IndexType>::buildRender(RenderBuffer_<T, IndexType>& buffer) {
    buffer.clear();
    if (!root_) return;
...
template<typename T, typename GasT, typename IndexType>
void Octree<T, GasT, IndexType>::buildRenderNodeAt(OctreeNode_<T, GasT, IndexType>* node, RenderBuffer_<T, IndexType>& buffer, uint32_t nodeIdx, const std::unordered_map<int, std::shared_ptr<GridObject>>& localObjects) {
...
template<typename T, typename GasT, typename IndexType>
const RenderData_<T, IndexType>* Octree<T, GasT, IndexType>::fastVoxelTraverse(const RenderBuffer_<T, IndexType>& buffer, const Ray& ray, float maxDist) {
